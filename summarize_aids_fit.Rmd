...
```{r long-compare-models include=FALSE}
source('analyze_aids_data.longitudinal.R')
...
Finally we look at the longitudinal submodel coefficients.

```{r show-coefs-long}
bayesplot::mcmc_areas(as.array(f7), regex_pars = '^Long1\\|[^(Intercept)|sigma]') + 
  bayesplot::vline_0() +
  ggtitle('Posterior parameter estimates for model fit with stan_jm',
          subtitle = 'Longitudinal submodel for sqrt(CD4) change over time (obstime)')
```
